  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-dark flex items-center justify-center p-6">
        ...
        >
          <h1 className="font-serif text-3xl mb-8 text-center text-dark">
            Admin Login
          </h1>
          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-2">
              <label className="font-sans text-xs uppercase tracking-widest text-dark/40">
                Access Code
              </label>
              <input
                type="password"
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-dark/5">
                <tr>
                  <th className="p-4 font-sans text-xs uppercase tracking-widest text-dark/40">
                    Name
                  </th>
                  <th className="p-4 font-sans text-xs uppercase tracking-widest text-dark/40">
                    Message
              ...
              <tbody className="divide-y divide-dark/5">
                {wishes.map((wish) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-xs text-dark/40">
...
const StatCard = ({ icon, label, value, color }: any) => (
  <div className="bg-white p-8 rounded-sm shadow-lg border border-dark/5">
    <div
      className={`w-12 h-12 rounded-full flex items-center justify-center mb-6 bg-white shadow-sm ${color}`}
    >
      {icon}
    </div>
    <p className="font-sans text-xs uppercase tracking-widest text-dark/40 mb-1">
      {label}
    </p>
